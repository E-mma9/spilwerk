# Onderhoudsovereenkomst

Voor zelfstandigen of kleine kantoren die je **maandelijks terugkerend** willen huren — typisch 1× per maand een check-in van 30 min remote, plus voorrang bij urgente vragen. Past op 2 A4. Voor jou als ZZP'er nuttig omdat het **voorspelbare cashflow** geeft.

> ⚠️ Aansluitvariant. Pas tarieven en uren-bundel aan per klant.

---

## Template

```markdown
---

# Onderhoudsovereenkomst Spilwerk · [Klantnaam]

**Spilwerk** (handelsnaam Emmanuel Tekle)
KvK: [nummer] · BTW: [nummer]
Adres: [adres] · E-mail: emmanueltekle@gmail.com

en

**[Klant bedrijfsnaam]**
T.a.v. [contactpersoon]
KvK: [klant KvK-nummer]
Adres: [klant adres]

## 1. Wat dit abonnement inhoudt

Op maandelijkse basis levert Spilwerk:

- **[X] uur op afstand werk per maand** (besteding naar eigen invulling van klant: troubleshooting, kleine wijzigingen, advies, onderhoud)
- **Voorrang bij urgente vragen**: reactie binnen 4 werkuren op werkdagen
- **Eens per kwartaal** een korte check-in (15-30 min videocall) over wat er goed/niet-goed loopt
- **Korting op nieuwe projecten**: 10% op vaste-prijs klusjes voor abonnement-houders

## 2. Wat dit abonnement NIET inhoudt

- 24/7 oncall — werkdagen 9:00-18:00
- Hardware-vervanging (gaat tegen normale tarieven)
- Werk op locatie (eigen tarief €60 voorrijden + uurtarief, buiten Twente +€30)
- Implementatie van grote nieuwe systemen (apart project)

## 3. Tarief

| | Bedrag (excl. btw) |
|---|---:|
| Maandtarief (X uur remote inbegrepen) | € [bedrag] |
| Btw 21% | € [bedrag] |
| **Totaal per maand incl. btw** | **€ [bedrag]** |

**Voorbeeld-tarieven:**
- 1u/mnd inclusief: €45/mnd
- 2u/mnd inclusief: €85/mnd
- 4u/mnd inclusief: €160/mnd
- 8u/mnd inclusief: €300/mnd

**Boven de bundel:**
- Extra remote-werk: €45/u (zonder kwartier-tellen, ronding op halve uren)
- Werk op locatie: €60 voorrijden (buiten Twente +€30) + €60/u

**Niet-gebruikte uren** verschuiven 1 maand vooruit, vervolgens vervallen ze (geen onbeperkte stapeling).

## 4. Betaling

- Maandelijkse facturatie, vooraf op de 1e van de maand
- Automatische incasso of overschrijving
- Betalingstermijn: 14 dagen

## 5. Looptijd en opzegging

- Begindatum: [datum]
- Looptijd: doorlopend, **stopzegging per email** met inachtneming van **1 maand opzegtermijn**
- Geen minimum-contractperiode — opzeggen kan vanaf de eerste maand

## 6. Geheimhouding

Spilwerk verplicht zich tot geheimhouding van alle vertrouwelijke informatie van [Klant], onverminderd ook na beëindiging van deze overeenkomst voor een termijn van 5 jaar.

## 7. Aansprakelijkheid

Op deze overeenkomst zijn de algemene voorwaarden van Spilwerk versie [datum] van toepassing. Aansprakelijkheid is begrensd zoals daarin omschreven, met een maximum per gebeurtenis gelijk aan **3 maand-tarieven**.

## 8. Verwerkersovereenkomst

Indien Spilwerk in het kader van dit abonnement persoonsgegevens verwerkt namens [Klant] (bv. bij e-mail-beheer, mailbox-toegang, klantenadministratie) wordt aanvullend de verwerkersovereenkomst van Spilwerk versie [datum] afgesloten.

## 9. Wijzigingen

Tariefswijzigingen worden minimaal **1 maand voorafgaand** aan de ingangsdatum schriftelijk gemeld. Klant heeft dan recht op opzegging zonder kosten.

## 10. Toepasselijk recht

Nederlands recht. Geschillen bij voorkeur in onderling overleg; bij geen oplossing de bevoegde rechter te [Almelo / Enschede / Zwolle].

---

**Aldus overeengekomen te [plaats], op [datum].**

| Voor [Klant bedrijfsnaam] | Voor Spilwerk |
|---|---|
| Naam: __________________ | Naam: Emmanuel Tekle |
| Datum: _________________ | Datum: _________________ |
| Handtekening: __________ | Handtekening: __________ |
```

---

## Wanneer aanbieden?

Bied dit pas aan **na** de derde of vierde reguliere klus bij dezelfde klant. Niet eerder. Reden:

- Klant moet je ritme, betrouwbaarheid en stijl al kennen
- Jij moet hun stack al kennen om realistisch in te schatten hoeveel uren/maand
- Voortijdig aanbieden = klant denkt "verkooppraatje, alleen om vast inkomen te krijgen"

## Wat het je oplevert

| Aantal abonnementen | Bruto recurring per maand | Effectief uren-investering |
|---|---:|---:|
| 5× 1u/mnd-abonnement | €225 | 5u/mnd |
| 3× 2u/mnd + 5× 1u/mnd | €480 | 11u/mnd |
| 5× 2u/mnd + 5× 1u/mnd | €650 | 15u/mnd |

**Voor jou interessant:** 5-10 abonnement-klanten = €300-700/mnd recurring zonder voorrijden. Compenseert de verlies-uren door reizen bij eenmalige bezoeken. Op termijn een goede basis.

**Niet meer dan 10 abonnement-klanten** — anders ben je fulltime alleen onderhoud aan het doen en heb je geen tijd voor nieuwe klusjes.
