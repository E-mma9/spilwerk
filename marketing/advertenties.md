# Kant-en-klare advertentieteksten

Canonieke, plak-klare teksten voor de drie handmatige kanalen. **Tarieven en telefoonnummer hier synchroon houden met `CLAUDE.md`.** De wekelijkse marketing-Routine gebruikt deze als basis en varieert de openingszin per week.

Huidige kernfeiten: 1e uur aan huis in Twente €60 (incl. voorrijden), buiten Twente +€30, daarna €15/kwartier, remote €45/u, klein remote-klusje €30, avond/weekend +25%, werkt-het-niet €0. Telefoon: 06-87433537. Werkgebied: heel Overijssel.

> **Accounts:** log op Marktplaats in met `emmanueltekle@gmail.com` (het `spilwerk@gmail.com`-account is verwijderd en ligt onder bezwaar). Zet je Marktplaats-weergavenaam op "Spilwerk". Marktplaats staat los van Google Business Profile.

---

## 1. Marktplaats

**Categorie:** Diensten en Vakmensen → Computer en internet aan huis
**Titel** (max 60 tekens): `IT-hulp aan huis — heel Overijssel — werkt niet? geen kosten`
**Prijstype:** "Zie omschrijving"
**Plaats:** Enschede

**Beschrijving:**

```
Computer traag? Wifi haalt de slaapkamer niet? Telefoon doet raar, of moet je nieuwe laptop nog ingesteld worden? Ik kom gewoon bij je langs.

Vriendelijke IT-hulp aan huis, in normaal Nederlands — ik leg rustig uit wat ik doe, geen jargon, geen verrassingen achteraf.

Wat ik vaak oplos:
✓ Trage of vastlopende computer, virus, herinstallatie
✓ Wifi en internet door het hele huis
✓ Telefoon en tablet instellen, foto's overzetten
✓ Foto-backup zodat je niks kwijtraakt
✓ Veilig online: wachtwoorden, phishing, tweestapsverificatie
✓ Ook voor zelfstandigen en kleine kantoren

Tarieven — helder:
• Bezoek aan huis in Twente (1e uur incl. voorrijden): € 60
• Buiten Twente (Zwolle, Deventer, Kampen e.o.): + € 30 voorrijden
• Daarna per kwartier: € 15
• Hulp op afstand: € 45/uur
• Werkt het niet? Dan betaal je niets.

Werkgebied: heel Overijssel.
Even bellen of appen wat er speelt kan altijd — eerste contact is gratis.

📞 06-87433537 — Emmanuel, Spilwerk
```

**Kosten:** de categorie "Diensten en Vakmensen" is betaald op Marktplaats (geen gratis optie). Kies **Basis à €9,90** (4 weken zichtbaar) — Plus/Premium niet waard tot je weet dat het kanaal klanten oplevert. Eén klant = €60, dus €9,90 verdient zich makkelijk terug.

**Onderhoud:** met Basis staat de advertentie 4 weken vast online — dus NIET tussentijds verwijderen en herplaatsen (dubbel betalen). Na 4 weken evalueren: leverde het aanvragen op? Zo ja → opnieuw plaatsen (evt. Plus proberen). Zo nee → energie naar Google Business Profile + buurt. (Het "elke 7–10 dagen verversen"-advies uit tracker.md geldt alleen voor gratis advertenties, niet voor deze betaalde categorie.)

---

## 2. WhatsApp Business (profiel)

Aparte gratis WhatsApp Business-app, kan op hetzelfde nummer 06-87433537.

- **Bedrijfsnaam:** Spilwerk
- **Categorie:** Computerservice
- **Servicegebied:** heel Overijssel (kies "klanten in een bepaald gebied", géén vast winkeladres)
- **Openingstijden:** ma–vr 09:00–18:00 · za–zo op afspraak
- **E-mail:** emmanueltekle@gmail.com
- **Website:** spilwerk.nl (zodra live)

**Bedrijfsomschrijving** (max ~256 tekens):

```
Vriendelijke IT-hulp aan huis in heel Overijssel. Trage computer, wifi-gedoe, telefoon of backup — ik kom langs, leg rustig uit wat ik doe, geen jargon. Werkt het niet? Dan betaal je niets. Bel of app gerust.
```

**Welkomstbericht** (Bedrijfstools → Berichten):

```
Hoi! Bedankt voor je bericht. Vertel kort wat er speelt (bv. "laptop is traag" of "wifi haalt de slaapkamer niet"), dan laat ik je snel weten of ik kan helpen en wat het ongeveer kost. — Emmanuel, Spilwerk
```

**Afwezigheidsbericht** (buiten openingstijden):

```
Bedankt voor je bericht! Ik lees het en reageer op werkdagen meestal binnen een paar uur. Spoed? Bel gerust: 06-87433537. — Emmanuel
```

---

## 3. Facebook — introductiepost

> ⚠️ **Lees eerst de groepsregels.** Alleen plaatsen in groepen die "vraag & aanbod", "diensten" of "ondernemers" toestaan. In gewone buurtgroepen: niet plaatsen, wel inhoudelijk reageren op IT-vragen. Foto erbij verdubbelt het bereik.

```
Hoi allemaal 👋

Ik ben Emmanuel uit Enschede en ik help mensen aan huis met computer- en internetgedoe — onder de naam Spilwerk, in heel Overijssel.

Denk aan: een trage laptop, wifi die de slaapkamer niet haalt, een nieuwe telefoon instellen, foto's veiligstellen, of gewoon iemand die het rustig uitlegt zonder moeilijke woorden.

Ik werk overdag als systeembeheerder en deed dit werk eerder bij Student aan Huis, dus je zit goed. En eerlijk is eerlijk: lukt het niet, dan betaal je niets.

Eerste uur aan huis in Twente €60 (incl. voorrijden), daarbuiten in Overijssel iets meer reiskosten. Even bellen of appen wat er speelt kan altijd, gratis en vrijblijvend.

📞 06-87433537

Mocht je iemand kennen die hulp kan gebruiken — je ouders, een buur, een collega — voel je vrij dit te delen. Dank alvast! 🙂
```
